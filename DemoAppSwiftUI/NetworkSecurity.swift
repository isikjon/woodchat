...
enum WoodChatPinning {
    ...
    ///
    /// Проверено 05.09.2026 отдельным Swift-скриптом (та же функция spkiHash,
    /// та же связка SecCertificate/SecKey) против живой цепочки сервера —
    /// значения совпадают, пиннинг не является причиной проблем со входом.
    /// (Первая проверка через openssl показала расхождение, но это оказалась
    /// ошибка сравнения: DER от openssl не побайтово совпадает с тем, что
    /// собирает сам код из SecKeyCopyExternalRepresentation + заголовка.)
